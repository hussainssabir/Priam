...
public class RestoreTokenSelector {
    ...
    public BigInteger getClosestToken(String tokenToSearch, Date startDate) {
        BigInteger tokenToSearchInteger =
                parseToken(
                        tokenToSearch,
                        "Instance does not have a valid, single token to find the closest backup to. Token: %s");
        List<BigInteger> tokenList = new ArrayList<>();
        Iterator<AbstractBackupPath> iter = fs.listPrefixes(startDate);
        while (iter.hasNext()) {
            BigInteger backupToken =
                    parseToken(
                            iter.next().getNodeIdentifier(),
                            "Backup is not identified by a token. BackupNodeIdentifier: %s");
     ...
     * @param region Region to search for a node with the closest token
     * @return Token as BigInteger
     */
    public BigInteger getClosestToken(String tokenToSearch, String appName, String region) {
        ...
        for (PriamInstance ins : plist) {
            if (ins.getDC().equalsIgnoreCase(region)) {
                BigInteger otherNodeToken =
                        parseToken(
                                ins.getToken(),
                                "Remote instance does not have a valid, single token to find the closest backup to. Token: %s");
